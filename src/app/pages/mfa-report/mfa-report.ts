...
import { MfaReportService } from '../../services/mfa-report.service';
import { MfaReportEntry } from '../../models/mfa-report.model';

@Component({
  selector: 'app-mfa-report',
  standalone: true,
  imports: [CommonModule, Loader],
  templateUrl: './mfa-report.html',
  styleUrls: ['./mfa-report.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MfaReport implements OnInit, OnDestroy {
  entries: MfaReportEntry[] = [];
  loading = false;
  ...
  constructor(
    ...
    private reportService: MfaReportService,
  ...
  ngOnInit(): void {
    this.companySelection.selectedCompanyId$.pipe(takeUntil(this.destroy$)).subscribe((id) => {
      ...
      if (!this.activeCompanyId) {
        ...
        this.loading = false;
        this.error = 'Select an AR company from the navbar to view MFA report.';
        this.detectChanges();
        return;
      }
  ...
  exportCsv() {
    ...
    const headers = ['AR Company Code', 'User Name', 'Email', 'Role Name', 'MFA Status'];
    const data = this.entries.map((entry) => [
      this.companyCode || 'N/A',
      entry.userName || '--',
      entry.email || '--',
      entry.roleName || '--',
      this.formatStatus(entry.mfaStatus),
    ]);
    ...
    this.downloadBlob(new Blob([csvRows], { type: 'text/csv;charset=utf-8;' }), this.fileName('csv'));
  }
  ...
  exportExcel() {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'MFA Report');
  ...
  exportPdf() {
    ...
    const headers = ['AR Company Code', 'User Name', 'Email', 'Role Name', 'MFA Status'];
    const baseWidths = [
      Math.max(usableWidth * 0.15, 30),
      Math.max(usableWidth * 0.2, 40),
      Math.max(usableWidth * 0.25, 60),
      Math.max(usableWidth * 0.2, 40),
    ];
    const remainingWidth = Math.max(
      usableWidth - baseWidths.reduce((sum, width) => sum + width, 0),
      40,
    );
    ...
    doc.setFontSize(16);
    doc.text('MFA Report', margin, margin + 4);
    doc.setFontSize(10);
    ...
    this.entries.forEach((entry) => {
      ...
      const rowHeight = Math.max(
        ...wrapped.map((lines) => Math.max(lines.length, 1) * lineHeight + 4),
        12,
      );
    ...
    this.reportService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.loading = false;
          this.error = this.entries.length ? null : 'No MFA records found for this company.';
          this.detectChanges();
        },
        error: (error) => {
          if (this.activeCompanyId !== companyId) return;
          ...
          this.error =
            error?.error?.message ||
            error?.message ||
            'Unable to load the MFA report at this time.';
          this.detectChanges();
        },
      });
  }
  ...
  private fetchCompanyCode(companyId: number) {
    this.companyService
      ...
      .subscribe({
        next: (response) => {
          this.companyCode = response?.data?.companyCode || null;
          this.detectChanges();
        },
        error: () => {
          this.companyCode = null;
  ...
  private fileName(ext: string) {
    const timestamp = new Date()
      ...
      .replace('T', '_')
      .split('.')[0];
    return `mfa-report_${timestamp}.${ext}`;
  }
